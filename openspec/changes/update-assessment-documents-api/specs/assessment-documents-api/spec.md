---
capability: assessment-documents-api
---

## ADDED Requirements

### Requirement: Documents base path and ownership
The assessment-service MUST expose the documents feature on `/api/v1/documents` and scope all operations to the authenticated user.
#### Scenario: Access denied to another user's document
- **WHEN** a user calls any `/api/v1/documents/{id}` endpoint for a document they do not own
- **THEN** the service returns an authorization failure (e.g., `404 Not Found`) without revealing the resource.
...
The system MUST allow users to create a document via `POST /api/v1/documents` with required `title`, `content`, and `category` plus optional `course` and `tags`.
